...
export class AdminComponent implements OnInit {
  ...
  selectCreature(creatureId: number) {
    ...
    this.newCreature.displayId = tempCreature?.displayId
      ? tempCreature.displayId
      : 0;
    this.newCreature.name = tempCreature?.name ? tempCreature.name : '';
    this.newCreature.imageUrl = tempCreature?.imageUrl
      ? tempCreature.imageUrl
      : '';
    this.newCreature.attack = tempCreature?.attack ? tempCreature.attack : 0;
  }

  addNewCreature() {
    if (this.newCreature.attack == 0 || this.newCreature.name == '') {
    ...
    this.adminService.addNewCreature(this.newCreature).subscribe(
      (data: any) => {
        ...
        this.adminService.getAllCreatures();
      },
      (error: any) => {
